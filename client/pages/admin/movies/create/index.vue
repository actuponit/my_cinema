  ...
  <div class="container mx-auto p-6 flex-1">
    ...
    <form @submit="onSubmit" methd="post">
      <div class="p-6">
        ...
        <div class="grid grid-cols-2 gap-6">
          <UFormGroup label="Title" name="title" v-bind="titleProps" class="space-y-4">
            <UInput name="title" v-model="title" type="text"
          ...
          <UFormGroup label="Genre" name="genere" v-bind="genreProps" class="space-y-4">
            <UInputMenu name="genre" v-model="genre" :options="genres"
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
          </UFormGroup>
          <UFormGroup :error="durationProps.error" class="relative space-y-4">
            <template #label>
              ...
              <div class="absolute top-0 right-0 flex gap-4">
                <UButton icon="i-heroicons-plus" @click="updateDuration(5)" :disabled="duration >= 14400" />
                <UButton icon="i-heroicons-minus" @click="updateDuration(-5)" :disabled="duration <= 900" />
              </div>
            </template>
            <template #default>
          ...
          <UFormGroup label="Thumbnails" :error="thumbnailsProps.error"
            help="The image must be less than 10mb and an image format">
          ...
          <UFormGroup label="Director" :error="directorProps.error" class="space-y-4">
            <USelectMenu name="director" value-attribute="id" option-attribute="name" v-model="director"
              :searchable="searchDirctors" :loading="loadingDirectors" :searchableLazy="true" :debounce="400" />
          </UFormGroup>

          <UFormGroup class="space-y-4" label="Actors" :error="actorsProps.error">
            <USelectMenu v-model="actors" name="actors" id="actors" :options="acotrsOptions" value-attribute="id"
              option-attribute="name" v-model:query="arg" searchable :loading="loadingActors" :searchableLazy="true"
              :debounce="400" multiple />
          </UFormGroup>
          <div class="col-span-2 bg-gray-800 p-4 text-lg mt-4 mb-2 font-bold border-b border-b-gray-50">
            Description
          </div>
          <UFormGroup label="Discription" name="description" :error="descriptionProps.error"
            class="space-y-4 col-span-2">
            <UTextarea name="description" v-model="description" autoresize
              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
          </UFormGroup>
          <div class="col-span-2 bg-gray-800 p-4 text-lg mt-4 mb-2 font-bold border-b border-b-gray-50">
            Schedule
      ...
      <button type="submit" @click="onSubmit"
        class="inline-flex justify-center py-2 px-4 border justify-self-end ml-auto border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
        Save Movie
      </button>
    </form>
  </div>
