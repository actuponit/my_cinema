<template>
  <div class="container mx-auto px-4 py-8 flex-1">
    ...
    <div v-if="bookmarks.length > 0">
      <USelect v-model="sortBy" :options="sortOptions" placeholder="Sort by" class="mb-6 max-w-xs" />

      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        <div v-for="movie in bookmarks" :key="movie.id" class="relative">
          <MovieCard :movie="movie" />
          <UButton color="gray" variant="ghost" icon="i-heroicons-trash" size="sm"
    ...
    <ErrorComponent v-else icon="i-heroicons-bookmark" title="No bookmarks yet"
      message="You haven't bookmarked any movies. Start exploring and save your favorites!">
      <template #action>
        <UButton color="primary" @click="goToMovies">
          Explore Movies
        </UButton>
      </template>
    </ErrorComponent>
