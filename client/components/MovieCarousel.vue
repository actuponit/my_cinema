                ...
                <div class="bg-gray-800 rounded-lg overflow-hidden shadow-xl">
                    ...
                    <div class="lg:p-8 p-2 flex-1">
                        ...
                        <ul class="mt-2 space-y-1">
                            <li v-for="(time, timeIndex) in movie.showtimes" :key="timeIndex" class="text-gray-400 flex justify-between items-center border-b-gray-700 border-b-2 py-1">
                                {{ time }}
                                <button class="mt-6 px-4 py-1 bg-gray-700 text-white  font-semibold rounded hover:bg-opacity-90 transition duration-300">
                                    Book Now
                                </button>
                            </li>
                        </ul>
  ...
  const movies = [
    {
      ...
      showtimes: ["MON, 12:30 PM",]
    },
  ];
  
  const nextSlide = () => {
    currentSlide.value = (currentSlide.value + 1) % movies.length;
  };
  
  const prevSlide = () => {
    currentSlide.value = (currentSlide.value - 1 + movies.length) % movies.length;
  };
