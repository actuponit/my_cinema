...
CREATE OR REPLACE FUNCTION bookmark_movie(movie_row movies, hasura_session json)
RETURNS boolean AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1
        FROM bookmarks A
        WHERE (hasura_session ->> 'x-hasura-user-id')::int = A.user  AND A.movie_id = movie_row.id
    );
END;
